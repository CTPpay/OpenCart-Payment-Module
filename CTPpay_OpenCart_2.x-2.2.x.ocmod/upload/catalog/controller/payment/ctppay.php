<?php
class ControllerPaymentCtpPay extends Controller
{
    public function index()
    {
		...
		$data['ref'] = $this->url->link('payment/ctppay/callback', '', true);

        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/ctppay.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/ctppay.tpl', $data);
		} else {
			return $this->load->view('default/template/payment/ctppay.tpl', $data);
        }
