...
class ControllerExtensionPaymentCtpPay extends Controller {
	public function index() {
		...
		$data['key'] = $this->config->get('payment_ctppay_merchant_key');
		...
		return $this->load->view('extension/payment/ctppay', $data);
	}
	...
	public function callback() {
		...
		if (isset($this->request->post['data'])) {
			...
			$securkey = $this->config->get('payment_ctppay_secure_key');
			...
			if ($status == "Confirmid") {
				...
				if($order_info['total']==$total && $order_info['currency_code'] == $currency) {
					$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_ctppay_order_status_id'));
				}
			}
